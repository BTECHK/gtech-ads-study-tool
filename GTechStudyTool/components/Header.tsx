...
export const keyProcesses = [
  ...
  { id: 'bidding-systems', name: 'Bidding Strategies & AI/ML', criticality: 'must_know_cold', relatedNodes: ['phase-2', 'p2-3', 'p2-4', 'p2-5'] },
  { id: 'campaign-types', name: 'Google Ads Campaign Types', criticality: 'must_know_cold', relatedNodes: ['phase-1', 'phase-2'] },
  { id: 'ads-lifecycle', name: 'End-to-End Ads Lifecycle', criticality: 'must_know_cold', relatedNodes: ['phase-1', 'phase-2', 'phase-3', 'phase-5', 'phase-7'] },
];
...
export function Header() {
  const {
    ...
    expandAll, addToExpanded, collapseAll,
    setSelectedNodeId,
  ...
  const handleKeyProcessSelect = (process: typeof keyProcesses[0]) => {
    ...
    // Extract parent phases from relatedNodes (e.g., 'p5-1' -> 'phase-5')
    const parentPhases = new Set<string>();
    process.relatedNodes.forEach(nodeId => {
      // Match pattern like 'p5-1' or 'phase-5'
      const subProcessMatch = nodeId.match(/^p(\d+)-/);
      if (subProcessMatch) {
        parentPhases.add(`phase-${subProcessMatch[1]}`);
      }
      // Also check if it's already a phase
      if (nodeId.startsWith('phase-')) {
        parentPhases.add(nodeId);
      }
    });

    // Add parent phases AND related nodes to expanded set (don't replace)
    const nodesToExpand = [...parentPhases, ...process.relatedNodes];
    addToExpanded(nodesToExpand);
  ...
  return (
    <header className="min-h-[64px] bg-white border-b flex flex-wrap items-center justify-between px-4 gap-2 sticky top-0 z-50">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <h1 className="text-base font-semibold text-gray-900 whitespace-nowrap">GTech Digital Lifecycle Study Tool</h1>
            ...
            <>
              ...
              <div className="absolute top-full left-0 mt-1 w-80 bg-white border rounded-lg shadow-xl z-50 max-h-[70vh] overflow-auto">
                <div className="p-2 border-b bg-gray-50">
                  <p className="text-xs font-medium text-gray-500">18 Critical Interview Processes</p>
                </div>
                <div className="p-1">
                  {selectedProcess && (
                    <Button
